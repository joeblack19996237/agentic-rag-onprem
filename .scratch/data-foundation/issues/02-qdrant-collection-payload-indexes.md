...
> Source: `specs/10-build-plan.md` TASK-007 (Phase 2). Traces to `specs/02-requirements.md` REQ-003, NFR-003 and `specs/13-decision-log.md` DEC-059, DEC-086, DEC-142. Physical schema source: `specs/07-database.md`'s Qdrant Collections section. Verification Pattern: TDD-Exempt — Infrastructure-as-Code.
...
One Qdrant collection, named `<corpus_id>_<embedding_model_version>` (e.g. `default_bge-m3-v2`, per DEC-059's double-collection convention), configured with both dense and sparse vectors (`bge-m3` dense output + a separate model's sparse output, DEC-086/**DEC-142** — a collection with only a dense vector is not valid). Five mandatory payload indexes created before any ingest can write to it: `allow_principals`, `deny_principals`, `retention_state`, `document_id`, `security_label` (all keyword indexes, per `specs/07-database.md`'s Payload Indexes table).

**Addendum (DEC-142, 2026-07-15, no code change needed)**: this issue's original text attributed the sparse vector to "`bge-m3` dense + lexical output" — TEI cannot actually serve `bge-m3`'s own sparse output (DEC-142 corrects DEC-086). This issue's own deliverable (the collection's dense+sparse vector *configuration*) is unaffected — Qdrant's `SparseVectorParams()` carries no model-specific config, so `ingest/qdrant_setup.py`'s existing `create_collection()` needs no change regardless of which model actually produces the sparse vector at ingest time (that's `document-ingest-pipeline`'s concern, `ingest/embedding.py`). Noted here only so this file's own description doesn't misattribute the source.
