...
- [ ] CI runs the same test and shows it failing for the same reason (not just locally)
      ...
      **Pending**: `.github/workflows/ci.yml` now has a dedicated step ("Smoke test (GET /ready) — expected red at this phase") running `pytest tests/api/test_ready.py -v`, so pushing will actually exercise this. Awaiting user confirmation before `git push`.
