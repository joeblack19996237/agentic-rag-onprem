...
- **Added 2026-07-13, `specs/13-decision-log.md` DEC-135**: the "Acceptance test" line under Testing Decisions ("end-to-end ingest... retrievable via a direct Qdrant query") is a Tier-3 (live-service) criterion under DEC-135's verification tiering — this agent execution environment has no Docker, so it cannot itself stand up a real Qdrant instance to run that test. When this PRD is broken into issues (`to-issues`), split it per DEC-135/`verifiable-acceptance-criteria`: an agent-checkable Tier-1 proxy (e.g. a fake/in-memory index double asserting the same payload-shape and partial-write-atomicity behavior the real acceptance test checks) plus a `[manual-verify]` item naming an owner and the exact `docker compose up` + direct-query command for the real Qdrant check. The two TDD Red items above (lines 48-49) are already Tier-1/Tier-2 by construction (status transitions via `TestClient`, `LocalAdapter` for ACL) and need no change.
- **Updated 2026-07-14, `specs/13-decision-log.md` DEC-140, corrected same-day by DEC-141**: narrows the note directly above, but not as broadly as first recorded. `qdrant-client` (already pinned) ships an embedded local mode (`QdrantClient(":memory:")`/`path=...`) that runs the real client library in-process, no Docker needed — confirmed by actually running it, not just reading about it. When this PRD is broken into issues, the "retrievable via a direct Qdrant query" acceptance test's **retrieval/filter behavior** should run against a `QdrantClient(":memory:")` instance directly (real payload write + real filter query) — that part is genuinely stronger than a hand-rolled fake. **But this ingest pipeline also writes payload indexes as part of "indexed" (TASK-009's Layer 1 ACL enrichment step), and index *existence* cannot be checked via local mode at all** — `create_payload_index()` silently no-ops there (real `UserWarning`, empty `payload_schema` on introspection). The `docker compose up` manual-verify item stays scoped to index existence + production-scale recall, not narrowed away — only the retrieval/filter-query part of the acceptance test moved to agent-executable.
