...
|---|---|---|---|
| [01-product-brief.md](01-product-brief.md) | 32K / 305 lines | Problem, personas, MVP scope, non-goals, business model, success metrics, risk register | DEC-005, DEC-010, DEC-020, DEC-024, DEC-025, DEC-072, DEC-131, RC-T1-12, RISK-011 |
...
| File | Size | Covers |
|---|---|---|
| [13-decision-log.md](13-decision-log.md) | 162K / 148 lines (grew post-Stage-8; the 56K/92-line figure here predates DEC-109 onward and had gone stale) | All DEC-### entries + supersede history, through **DEC-142** (2026-07-15). **DEC-142** — corrects DEC-086's dense+sparse-single-model claim: TEI cannot serve `bge-m3`'s sparse output at all (not just ColBERT, as the 2026-07-13 cross-model review's R.26 finding partially caught) — sparse embedding moves to a dedicated SPLADE model, also via TEI, found and resolved while implementing `document-ingest-pipeline` Issue 01's embedding client. **DEC-135** — Phase 2+ verification-tiering strategy for the agent execution environment's Docker/GPU-less ceiling (**this row's own citation gap, missed by DEC-135's own re-audit note claiming it was updated, was the subject of cross-model review finding R.27** — fixed here). Cross-model-review-driven fixes (2026-07-13): **DEC-136** benchmark version sweep + Llama Guard 3-vs-4 evaluation; **DEC-137** concrete mechanism for DEC-091's KV-cache legal-hold invalidation; **DEC-138** softens DEC-134's Python 3.14 verification-confidence framing, adds `[manual-verify]` for Linux-target wheel/import check. **DEC-139** (2026-07-14) — performance tests (TEST-032/033/034/041) must implement regression detection against a persisted baseline, not just threshold-gating, once Phase 2+ implements them; nothing built yet, requirement pinned ahead of need. **DEC-140** (2026-07-14) — narrows DEC-135's Tier-3 bucket for Qdrant: `qdrant-client`'s embedded local mode makes collection/payload-index/filter-correctness criteria agent-executable, ahead of TASK-007's issue being drafted. **DEC-141** (2026-07-14) — corrects DEC-140: empirically, local mode's payload-index existence is not agent-checkable at all (silent no-op, empty introspection), only vector-config and filter-semantics genuinely moved; caught by actually running the check rather than trusting a web-sourced summary. **Most-referenced ids: DEC-082 (53), DEC-077 (53), DEC-052 (49), DEC-075 (47), DEC-042 (38)** — these are the load-bearing decisions as of the last full count; any change here has the widest blast radius. |
...
**Virtual slicing, not physical file splitting.** `04-architecture.md` stays a single source file (currently ~1227 lines and growing) — do not fork it into per-dimension files. Section numbers (`§7B.12`, `§8.1`, etc.) are cited as stable anchors from `02-requirements.md`, `13-decision-log.md`, `20-agent-behavior.md`, `23-evals-guardrails.md`, and `92-stage5-review-memos.md`; forking would break those references and risks re-introducing drift between copies. Instead, use `Read` with `offset`/`limit` to pull only the line range a dimension needs, plus the fixed Summary range (L7–20, L220–354) for orientation on every pass. `offset`/`limit` below are ready to pass to `Read` directly (offset = starting line, limit = line count).

**§8.1 is mandatory cross-cutting context** *(added 2026-07-05, after a Security-pass finding surfaced it)*: `04-architecture.md` §8.1 (`offset 807, limit 81`, "Verification pipeline") is the pipeline pseudocode that actually pins down node ordering — it belongs to no single dimension but silently constrains three of them (Security: where the retrieval-rail scan runs; Performance: the sequential-vs-parallel latency philosophy; Evaluability: mechanical/NLI threshold order). The Security pass (2026-07-05) found real drift specifically because §8.1 wasn't in its reading list: a first-draft fix invented a redundant `safety_retrieval/` node before noticing §8.1 already specified the ordering (see DEC-096). **Read §8.1 alongside the Summary range on every pass from here on**, not just the ones it's listed against below.

Note: offsets below were re-verified 2026-07-08 (`update-specs` skill, DEC-130 follow-up) by re-grepping `^## \|^### ` in `04-architecture.md` and `20-agent-behavior.md` and recomputing every cited range against the current headings. The prior 2026-07-05 re-verification had drifted by roughly 7-46 lines (larger drift toward the end of the file) after several intervening DECs (DEC-109, DEC-117, DEC-128, DEC-130, and others) edited `04-architecture.md` without a matching offset refresh. **If you edit `04-architecture.md` or `20-agent-behavior.md` again, re-grep `^## \|^### ` before trusting these numbers for the next pass** — this file has no automated staleness check, only this manual convention.

| # | Dimension | Primary source (offset/limit into `04-architecture.md` unless noted) | Notes |
|---|---|---|---|
| 0 | **Summary** — completeness & stability of the overall design | §1–2 elevator: `offset 7, limit 14`; §5 module map: `offset 220, limit 135` | Read first, and re-inject alongside every other pass below for orientation. ✅ Reviewed 2026-07-05 — 3 findings fixed (DEC-094, DEC-095). |
| 1 | **Security** | §4.3 SafetyRailAdapter: `offset 172, limit 48`; §7B JIT auth: `offset 483, limit 324`; **§8.1 verification pipeline: `offset 807, limit 81`**; §12.1–12.5: `offset 1094, limit 93`; plus `20-agent-behavior.md` §2.2–2.4 (`offset 79, limit 21`) | Highest blast radius — DEC-082/DEC-077 (53 refs each) live here. ✅ Reviewed 2026-07-05 — 1 finding fixed (DEC-096, DEC-097); required correcting a first-draft fix that conflicted with §8.1. |
| 2 | **Reliability / failure handling** *(added — not in original 9)* | `20-agent-behavior.md` §6: `offset 170, limit 24`; §9.4 concurrency: `offset 1020, limit 19`; §7B.4 circuit breaker: `offset 547, limit 18`; **§8.1 verification pipeline (reuse pass 1's extract — early-exit/circuit-breaker ordering lives here too): `offset 807, limit 81`** | Was previously scattered, not owned by any one doc. GPU OOM, model-server crash, ECM-unreachable degradation belong here explicitly. ✅ Reviewed 2026-07-05 — 3 findings fixed (DEC-098; adds NFR-030, NFR-031). |
| 3 | **Performance** | §7B.12 latency budget: `offset 772, limit 26`; §5.0 fan-out latency note: `offset 221, limit 7`; **§8.1 verification pipeline (reuse pass 1's extract — defines the parallel/sequential latency philosophy): `offset 807, limit 81`**; `23-evals-guardrails.md` thresholds (whole file — only 15K) | Cross-check against Reliability pass — timeout/circuit-breaker values should agree with latency budget. Cold-cache headroom is now thin (110 ms per DEC-097) — worth a dedicated look. ✅ Reviewed 2026-07-05 — 2 findings fixed (DEC-099). |
| 4 | **Deployment & cost** *(added — not in original 9)* | §4.2 hardware matrix + GPU/VRAM sub-sections: `offset 106, limit 66`; §9.1–9.6 deployment topologies: `offset 964, limit 93` | On-prem product — this is often the actual go/no-go dimension for a customer, not a performance sub-note. ✅ Reviewed 2026-07-05 — 2 findings fixed (DEC-100) plus 2 user-raised architecture questions resolved (DEC-101 docker-compose vs K8s; DEC-102 poll-only CDC mode, adds REQ-057/NFR-032). |
| 5 | **Maintainability & extensibility** | §3 build-approach decision: `offset 21, limit 45`; §5.1 call-direction rules: `offset 355, limit 26`; §5.1.1 typed state schema: `offset 381, limit 58` | Check that V2/V3 deferred items (§8.2, §8.3, §10.2–10.3) don't require re-architecting the MVP graph. ✅ Reviewed 2026-07-05 — 2 findings fixed (DEC-103). |
| 6 | **Evaluability** | `23-evals-guardrails.md` §2 + §7 (renumbered from duplicate §6 per DEC-099) (whole file — only 15K, no slicing needed); **§8.1 verification pipeline (reuse pass 1's extract — mechanical/NLI threshold ordering): `offset 807, limit 81`** | AI-specific dimension. Confirm every REQ with an acceptance criterion has a corresponding eval or is explicitly marked non-evaluable. ✅ Reviewed 2026-07-05 — 2 findings fixed (DEC-104). Judge-model naming gap (DEC-130) closed 2026-07-08 — see `23-evals-guardrails.md` §2.1. |
| 7 | **Traceability** | §12.4 logging: `offset 1151, limit 6`; `23-evals-guardrails.md` §3.3 (whole file); audit/ module notes inside §5.0 | Enterprise/compliance-facing — check `13-decision-log.md` for anything promoted "to MVP" here (e.g. DEC-060) actually landed in the module map. ✅ Reviewed 2026-07-05 — 2 findings fixed (DEC-105, DEC-106). |
| 8 | **API / interface contracts** | §7 intro + §7.1–7.4 (4 surfaces): `offset 454, limit 29`; `06-api-contracts.md` now exists as a full spec (Stage 7) | ✅ Reviewed 2026-07-05 — 1 finding fixed (DEC-107): §7 was missing 4 endpoints already referenced elsewhere; `06-api-contracts.md` stub created per this row's own instruction. |
| 9 | **ECM integration** | §7B full: `offset 483, limit 324`; cdc/ module notes inside §5.0 | Do last — depends on conclusions from Security (1), Reliability (2), and API (8). Overlaps Security's §7B range; that's expected (JIT auth *is* the ECM integration surface) — don't re-read, reuse pass 1's extract. ✅ Reviewed 2026-07-05 — 2 findings fixed (DEC-108) — both were propagation gaps from this same session's own DEC-102. **All 10 review-matrix rows now complete.** |
...
| 10 | `10-build-plan.md` | ~40 KB (grew post-Stage-8) | `TASK-001`..`TASK-039` (TASK-033..038 added Stage 8 audit fix; TASK-039 added post-Stage-8, DEC-129, NFR-007 restart-durability); TASK-029 extended post-Stage-8 (DEC-130, RAGAS judge model wiring + independence check; DEC-133, judge-model weight-presence AC); TASK-002/TASK-011 corrected post-Stage-8 (DEC-131, LangGraph version 0.2.x→1.2.x); TASK-002 extended post-Stage-8 (DEC-133, `llama-cpp-python` pin); TASK-002 re-pinned post-Stage-8 (DEC-134, Python version 3.12→3.14) | Every MVP REQ/NFR; applies RC-R2-X3 (team/solo annotation on every task) |
| 11 | `11-test-plan.md` | ~20 KB (grew post-Stage-8) | `TEST-001`..`TEST-041` (TEST-035..038 added Stage 8 fixes / DEC-128; TEST-039/040 added DEC-129; **TEST-041 added 2026-07-13, cross-model review R.17 — missing from this row until now, a citation gap this edit also closes**); AI Evals section's judge-decoupling prose corrected 2026-07-08 (DEC-130 — NLI decoupling and RAGAS judge decoupling are separate mechanisms, not one substituting for the other); Test Environments table gains an "Agent execution sandbox" row, distinct from "Local/CI" (DEC-135, 2026-07-13), narrowed for Qdrant's embedded local-mode client (DEC-140, 2026-07-14; corrected re: payload-index existence by DEC-141, same date); Performance Tests section gains a threshold-vs-regression-detection methodology note for TEST-032/033/034/041 (DEC-139, 2026-07-14) | `23-evals-guardrails.md` (golden set, cross-referenced not duplicated), `06-api-contracts.md`; closes RC-T8-01's remaining half |
