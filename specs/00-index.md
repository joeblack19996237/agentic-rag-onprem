...
| 10 | `10-build-plan.md` | ~40 KB (grew post-Stage-8) | `TASK-001`..`TASK-039` (TASK-033..038 added Stage 8 audit fix; TASK-039 added post-Stage-8, DEC-129, NFR-007 restart-durability); TASK-029 extended post-Stage-8 (DEC-130, RAGAS judge model wiring + independence check; DEC-133, judge-model weight-presence AC); TASK-002/TASK-011 corrected post-Stage-8 (DEC-131, LangGraph version 0.2.x→1.2.x); TASK-002 extended post-Stage-8 (DEC-133, `llama-cpp-python` pin) | Every MVP REQ/NFR; applies RC-R2-X3 (team/solo annotation on every task) |
