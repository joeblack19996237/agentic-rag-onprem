...
| Embedding | `bge-m3` via TEI (unified dense + sparse; **ColBERT-style multi-vector is a `bge-m3` capability but not exposed via TEI — corrected 2026-07-13, cross-model review R.26; TEI serves dense + sparse only, tracked as an open TEI feature request (`huggingface/text-embeddings-inference#141`), not a shipped mode**) | DEC-035 + **DEC-086 (reverts DEC-052's embedding narrowing)** | Separate container; single model produces the dense vector **and** the sparse vector REQ-003's hybrid retrieval requires — `bge-large-en-v1.5` (the interim English-only pick) is dense-only and has no sparse output, which silently broke hybrid retrieval until DEC-086. REQ-003's acceptance criteria are correctly limited to dense + sparse + rerank and never depended on ColBERT vectors |
