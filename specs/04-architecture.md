...
| Vector store | Qdrant single-node | DEC-034 (partial; vector store unchanged); dense + sparse + payload filter | One collection per `(corpus_id, embedding_model_version)` — double-collection naming convention at MVP per DEC-059; `corpus_id` = `documents.repository_id` per DEC-144 |
...
| Doc parsing | `pdfminer.six` + PyMuPDF + python-docx | DEC-036, **DEC-143** | **`pdfminer.six` primary + PyMuPDF rescue for PDF (corrected 2026-07-15 — Unstructured.io's PDF module has a hard, unconditional torch/OCR dependency, see DEC-143); python-docx direct for Word, unchanged. Unstructured.io itself has zero remaining role in either format.** **Open item (added 2026-07-16, DEC-147, second cross-model review R.8)**: DEC-143's own "Cost accepted" text only names the loss of Unstructured's structured-element output — `pdfminer.six`'s raw-text-extraction quality on CCM-style business documents (tables, headers/footers, multi-column layouts) versus Unstructured's has not been measured. Not a design change — `pdfminer.six` remains the correct pragmatic MVP choice — flagged for the first golden-set eval run to check empirically rather than left as a silent, unmeasured assumption |
...
Vector data lives in Qdrant; **one collection per `(corpus_id, embedding_model_version)`** at MVP so blue/green re-embedding (REQ-034 automation V2) creates a new collection without disturbing the live one. Schema is in place from day one per DEC-059; the automation pipeline is V2. `corpus_id` = `documents.repository_id` (DEC-144) — a repository is this schema's natural corpus boundary, not a separately-managed concept.
...
services:
  ...
  tei-embed:      # bge-m3 dense embedding (DEC-086)
  tei-embed-sparse: # dedicated SPLADE-architecture model, dense's sibling deployment (DEC-142, propagation completed DEC-146)
