...
| TEST-039 | Integration | NFR-024, DEC-129 | Cost-per-turn computation — a query's `cost_per_turn` metric observation matches `08-observability-logs.md`'s documented formula (`gpu_hourly_rate × gen_ai.request.duration / 3600`) at the default GPU-hourly rate; the golden-set eval run report (TASK-029) includes a cost-per-turn mean/p95 summary aggregated from these observations | A query with a known, controlled generation duration (mocked or measured); a golden-set smoke-ring run | Metric value matches the formula within measurement tolerance; eval report's cost summary is present and numerically consistent with the underlying per-query observations | Automated |
| TEST-040 | Integration | NFR-028, DEC-129 | vLLM optimization flags — `vllm.config` introspection after service startup shows `--enable-chunked-prefill` enabled unconditionally, and `--speculative-decoding` set per the tier default unless the measured-VRAM validation run (TASK-032) recorded < 4 GB free VRAM at peak concurrency, in which case it is disabled regardless of tier | vLLM service started at floor tier and at comfort tier; one run with a simulated low-VRAM validation result | Flag values match the documented tier/VRAM-conditional rule in both cases; a low-VRAM validation result overrides the tier default to disabled | Automated |
| TEST-043 | Integration | REQ-047, DEC-076 (added 2026-07-16, DEC-147, second cross-model review R.4) | General Redis cache hit/miss/TTL/force-refresh discipline (prompt/answer/ACL/embedding caches per DEC-076) — distinct from TEST-012, which covers only legal-hold invalidation of the KV-cache and answer-cache, not baseline hit/miss/TTL/force-refresh behavior for any cache | A repeated identical query within TTL (expect cache hit, no re-computation); the same query after TTL expiry (expect cache miss, re-computation, cache refreshed); an explicit force-refresh request (expect cache bypassed regardless of TTL) | Hit/miss/TTL/force-refresh each behave exactly as described, observable via a cache-hit-ratio metric or direct cache-backend inspection, not inferred from response latency alone | Automated |
...
| TEST-042 | Security | REQ-010, NFR-009 (TASK-040, added 2026-07-16, DEC-147, second cross-model review R.3) | Validly-signed JWT lacking the required admin scope/role claim, sent to every admin-surface route | `403` (not `401`, not `200`) on every route; a correctly-scoped JWT and the admin API key both continue to work unchanged |
...
**Threshold gate vs. regression detection (DEC-139, 2026-07-14)**: every row above states a fixed acceptance threshold — the hard bar a measurement must clear. None of them, as written, catch a measurement that stays under threshold while trending worse over time (e.g. TEST-032's p95 creeping from 3s toward the 8s ceiling across several releases would pass every run). Per DEC-139, whichever `TASK-###` implements TEST-032/033/034/041 must also persist each run's measured value and compare it against the last-recorded baseline, flagging a regression beyond a documented tolerance independently of whether the fixed threshold still passes. Not built yet — Phase 1 has no perf-sensitive code to measure (every module beyond `api/` is scaffold-only, `__init__.py` files with no implementation) and DEC-135 already places live-service performance testing in Tier 3 (`[manual-verify]`, out of this agent's current sandbox). This note exists so the threshold-vs-regression distinction isn't rediscovered under time pressure once Phase 2 implementation starts.

## AI Evals

Owned entirely by `23-evals-guardrails.md` — this file does not redefine RAGAS metrics, golden-set composition, or judge-decoupling policy. Cross-reference: `23-evals-guardrails.md` §2 (framework + golden set), §3 (guardrails), §3.3 (context fingerprint). **Judge decoupling (corrected 2026-07-08, DEC-130)**: two distinct mechanisms, both decoupled from the generation model, cover two distinct requirements. (a) The NLI model (`deberta-v3-base-mnli`, DEC-037) is architecturally distinct from the generation model and performs runtime grounding classification per claim — this satisfies REQ-005's citation-verification requirement, not the RAGAS judge-decoupling non-negotiable. (b) RAGAS's faithfulness/answer_relevancy scoring uses a separate LLM-as-judge, `Qwen2.5-14B-Instruct` (DEC-130) — a different vendor/family from both generation-model candidates, run offline via `cli eval run`, never GPU-resident. Prior wording here claimed the NLI model alone "satisfies the judge-decoupling requirement without needing a separate LLM-as-judge step" — that was a conflation of (a) and (b); DEC-130 corrects it by specifying (b) explicitly rather than treating (a) as a substitute for it.
