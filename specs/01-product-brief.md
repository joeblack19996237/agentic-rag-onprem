...
| **RISK-023** | **(Added 2026-07-16, `api-surface`/TASK-033 code review, DEC-145)** Admin-surface JWT auth verifies signature only, not scope — any correctly-signed JWT is currently accepted on admin endpoints, regardless of claims. Currently inert (no end-user JWT issuance path exists in this codebase yet), but the risk window is not gated by this project's own build-plan sequencing: it becomes live the moment any real vendor integration mints end-user JWTs against a live GroundedDocs install, since a non-admin-scoped-but-otherwise-valid token would then be obtainable and could read/modify admin data (documents, audit trail, config) | Low today (inert); High once any real vendor JWT integration exists | High (compliance + data exposure) | Architect | `TASK-040` (`10-build-plan.md`) adds scope/role claim enforcement to `require_auth`, tracked as a Phase 2 continuation of `TASK-033`, explicit precondition for any real (non-demo) deployment |
