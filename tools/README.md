...
- **Exit codes are the contract.** Document what 0 vs. non-zero means in the script's own docstring/header comment — a caller (agent or CI) should be able to branch on the exit code without reading the script's body. `tools/check_pypi_version.py`'s docstring is the reference example (0 vs. 1, each with a one-line meaning).
...
- **Actually run it before trusting it**, the same discipline as everything else in this repo (`CLAUDE.md`'s "Running and verifying this repo"). Writing a script and never running it is exactly the failure mode this whole convention exists to catch.
...
| `check_pypi_version.py` | Look up a package's actual current version on PyPI, or (with `--requires`) its declared dependencies — the one-line `urllib` check this repo's dependency-version-claims rule (`CLAUDE.md`) requires before pinning or citing a version/dependency footprint as current, promoted from an inline `python -c "..."` into something reusable. `--version X.Y.Z` targets a specific pinned release instead of latest (added 2026-07-15, DEC-143 — hand-rolled 3 times in one session checking `unstructured`'s PDF-extra dependency chain before pinning `pdfminer.six` instead) | `python tools/check_pypi_version.py <package>` — prints `<package>==<version>`; add `--requires` to print `requires_dist` instead, `--version X.Y.Z` to target a specific release; exit 1 if the lookup fails |
| `regenerate_openapi_snapshot.py` | Regenerate `tests/api/openapi_snapshot.json` from the live FastAPI app after adding/changing a route — promoted from an inline `python -c "..."` one-liner hand-rolled once per route-adding issue (TASK-033 Issue 02, then again Issue 03) without ever being written down, the cross-session-recurrence trigger this file's own "When to write a script" section describes | `python tools/regenerate_openapi_snapshot.py` — writes the snapshot and reminds the caller to hand-verify the diff against `specs/06-api-contracts.md`; exit 1 if `api.main.app` fails to import |
...
**Removed 2026-07-14**: `call_peer_review_model.py` (called a DeepSeek-compatible API in-session for `peer-review`'s model call). Deleted once the user confirmed peer review now runs externally, in a separate IDE/tool, the same standing pattern this project already used for `specs/` cross-model review (`.claude/skills/peer-review/specs-review-prompt.md`) — a script Claude Code invokes to call an external model is redundant once the model call itself happens outside this tool entirely. `.claude/skills/peer-review/SKILL.md` updated to match: it now assembles a portable review package instead of attempting the call itself.
