...
**Extends to capability claims, not just version claims.** A package importing cleanly, or a base install completing without error, doesn't mean the *specific* capability you're about to build against actually works — verify the exact code path (the real function/endpoint call), not just that the package/version resolves. Recurred twice within one week (2026-07-15): TEI's `/embed_sparse` endpoint doesn't serve `bge-m3` at all despite `/embed` (dense) working fine, confirmed only by reading the actual GitHub issue thread rather than trusting its title (DEC-142); Unstructured's base package imports cleanly, but its PDF module has a hard, unconditional import pulling in a full torch/OCR stack unrelated to plain text extraction (DEC-143). Both gaps were invisible to spec-writing and review rounds — including, in DEC-142's case, a dedicated cross-model review that cited the same source — and were only caught once implementation forced the actual call.
...
3. **Agent review, for a nontrivial diff** — before invoking it, re-read the source issue's own prose line-by-line against the diff first; `code-review`/`review` is a second gate, not a substitute for a first one. Recurred twice (2026-07-15): Issue 01's code review caught an unused `config/` query and a sync/async wording mismatch a careful self-read could plausibly have caught first; Issue 02's caught the issue text saying the job "requeues" when the actual implementation is an in-process blocking retry loop. Then run `code-review`/`review` plus `peer-review`; see `peer-review/SKILL.md`'s own trigger guidance.
