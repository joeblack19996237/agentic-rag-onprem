...
3. If any Testing Decision or User Story implies a future capability (e.g. "verify the deployed service handles production load") that this PRD's own scope doesn't build, move it to Out of Scope now — don't let `to-issues` inherit an assumption this PRD never actually delivered on.
